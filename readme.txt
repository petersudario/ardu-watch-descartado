Comandos para startar o reactjs e o mongo:

-- ReactJS: npm start ou yarn start
-- MongoDB: primeiro vá para a pasta backend usando "cd backend" no terminal e depois: npm run serve ou "yarn start"

Etapas para instalação da Node_modules:

// Instale um de cada vez para evitar erros

-- Parte React
1. cd ardu-watch
2. npm install --save
3. npm install axios react react-dom react-router-dom react-datepicker react-scripts bootstrap styled-components react-iconsComandos para startar o reactjs e o mongo:

-- ReactJS: npm start ou yarn start
-- MongoDB: primeiro vá para a pasta backend usando "cd backend" no terminal e depois: npm run serve ou "yarn start"

Etapas para instalação da Node_modules:

// Instale um de cada vez para evitar erros

-- Parte React
1. cd ardu-watch
2. npm install --save
3. npm install axios react react-dom react-router-dom react-datepicker react-scripts bootstrap styled-components react-icons